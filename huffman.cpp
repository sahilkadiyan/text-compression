...
struct Node
{
   ...
   int freq;
   char *code;
   Node *Llink;  //for left part of the binarytree
   Node *Rlink;    //for the right part of the binary tree

};

//here symbols are stored in the leaf nodes along with their frequencies
//as we know that for huffman coding there is tree(here we take binary tree for our simplicity) and the path from the root to the leaf node is the code for the particular symbol or character
//conventionally 0 is for left subtree nad 1 is for right subtree
...
//here we use priority queue i.e heaps in order to find out minimum from the binary tree
...
class minHeap
{
    private:
      ...
      public:						
        ...
        void insert(BinaryTree b);//for inserting the binary tree and then heapify the array of binary trees  
...
{
	minHeap Heap;
    //huffman tree is just like an inverted binary tree which is build from bottom to top
	//first of all from starting point to the ending point we start by picking up the minimum out of the all the charcters present
    //so we will get the symbols with lowest frequency at the bottom of the binary tree
    //here we take minimum because it leads to longer codes for lower frequency symbol i.e the intuition of huffman coding 
	...
	while (Heap.T[0].root->freq>1)
	{
		...
		cout<<"\nAfter insertion "<<l.root->freq<<"+"<<r.root->freq<<"= "<<HuffmanTree.root->freq<<endl;
		Heap.print();	
	}
...
//that function is basically used for decoding the binary tree
